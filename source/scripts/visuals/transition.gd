...
func show_transition(delay = 0.0, \
		time = _transition_time, rect_show_time = _transition_rect_show_time):
	var transition_time_half = time * 0.5
	hide_screen(delay, transition_time_half)
	show_screen(delay + transition_time_half + rect_show_time, transition_time_half)

func hide_screen(delay = 0.0, time = _transition_time):
	...
	tween.tween_property(_color_rect, "color", \
			solid_rect_color, time)
	tween.tween_callback(self, "emit_signal", ["transition_hid_screen"])

func show_screen(delay = 0.0, time = _transition_time):
	var solid_rect_color = Color(_color_rect.color.r, _color_rect.color.g, \
			_color_rect.color.b, 1.0)
	var clear_rect_color = Color(solid_rect_color.r, solid_rect_color.g, \
			solid_rect_color.b, 0.0)
